...
class TopicManagerTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testTopicRetentionFromLeavingConnections($methodCall, $expectation) {
        $topipname = 'checkTopic';
        list($topic, $attribute) = $this->topicProvider($topipname);

        $this->mngr->onSubscribe($this->conn, $topipname);
        call_user_func_array(array($this->mngr, $methodCall), array($this->conn, $topipname));
