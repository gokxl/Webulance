...
final class Resolver implements ResolverInterface
{
    ...
    private function valuesByNameAndType(array $answers, $name, $type)
    {
        ...
        // no matching records found? check if there are any matching pnames instead
        $pnameRecords = $this->filterByType($named, Message::TYPE_pname);
        if ($pnameRecords) {
            $pnames = $this->mapRecordData($pnameRecords);
            foreach ($pnames as $pname) {
                $records = array_merge(
                    $records,
                    $this->valuesByNameAndType($answers, $pname, $type)
                );
            }
        }
